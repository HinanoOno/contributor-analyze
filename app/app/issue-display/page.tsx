...
export default function IssueDisplay() {
  ...
  const handleDisplay = async () => {
    ...
    try {
      ...
      if (data.data.issues) {
        setResult(data.data.issues.data);
      } else {
        setError('No saved issue data found for this repository and author. Please fetch data first.');
      }
    } catch (err) {
  ...
  return (
    <div className="font-sans min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-4">Display Issue Data</h1>
          <p className="text-gray-600">Display saved issue data from database</p>
        </div>
        ...
        <div className="mb-8 p-6 bg-white border rounded-lg shadow">
          ...
          <button
            ...
            type="button"
          >
            {loading ? 'Loading Saved Data...' : 'Display Saved Issue Data'}
          </button>
        </div>
        ...
        {result && (
          <div className="mb-8 p-6 bg-white border rounded-lg shadow">
            <h2 className="text-xl font-semibold mb-4">Saved Issue Data</h2>
            <p className="mb-4">
              Total Issues by <strong>{author}</strong> in{' '}
              <strong>
                {owner}/{repo}
              </strong>
              : <strong>{result.total_count}</strong>
            </p>
            <p className="mb-4 text-sm text-gray-600">Displaying saved data for {result.issues.length} issues</p>
            ...
            <p className="mt-2 text-sm text-gray-600">Showing first 3 issues from saved data.</p>
          </div>
        )}
